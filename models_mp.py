...
class APPNP_MP(MessagePassing):
    ...
    def __init__(self, args, dataset,  K: int, alpha: float, dropout: float = 0.,
        ...
        super().__init__(**kwargs)
        self.dataset = dataset
        self.Q = torch.Tensor(int(dataset.data.edge_index.size(1) / 2), 1).to(args.device)
        self.gcn1_dual = nn.Linear(1, dataset.num_classes)
        self.K = K
        self.alpha = alpha
    ...
    def reset_parameters(self):
        self._cached_edge_index = None
        self._cached_adj_t = None
        self.gcn1_dual.reset_parameters()
    ...
    def forward(self, data, epoch, x: Tensor, edge_index: Adj,
        ...
        h = x
        Q_final = F.relu(self.gcn1_dual(self.Q))
    ...
    def __init__(self, args, K, dataset, bias=True, **kwargs):
        ...
        if self.Original_ot == 'ot' and self.A_F:
            if self.mlp_layers == 2:
                self.lin1 = Linear(dataset.num_classes, 32)
